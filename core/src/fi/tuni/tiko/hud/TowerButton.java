...
public class TowerButton extends Button {
    ...
    private final fi.tuni.tiko.gameObject.tower.Tower tower;
    final fi.tuni.tiko.gameObject.tower.TowerLocation location;
    ...
    public boolean onTouchDown(ScreenPosition position, int pointer) {
        if (IsInside(position.ToMenuPosition())) {
            if(GameLogic.getFunds() >= tower.getCost(data)) {
                fi.tuni.tiko.GameLogic.addFunds(-tower.getCost(data));
